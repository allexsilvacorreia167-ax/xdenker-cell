...
import { getProductsByCategories, ACESSORIOS_CATEGORIES } from "@/lib/queries";
import Link from "next/link";

const tabs: { label: string; value?: string }[] = [
    { label: "Todos" },
    { label: "Áudio", value: "audio" },
    { label: "Capas e Películas", value: "capas" },
];
...
export default async function AcessoriosPage({
    ...
    const products = await getProductsByCategories(ACESSORIOS_CATEGORIES);
    ...
    return (
        <>
            ...
            <main className="px-4 lg:px-8 py-6 pb-28 lg:pb-12 max-w-7xl mx-auto">
                <h1 className="text-2xl font-bold mb-4">Acessórios</h1>

                <div className="flex gap-2 mb-6 overflow-x-auto scrollbar-hide">
                    {tabs.map((tab) => (
                        <Link
                            key={tab.label}
                            href={tab.value ? `/acessorios?cat=${tab.value}` : "/acessorios"}
