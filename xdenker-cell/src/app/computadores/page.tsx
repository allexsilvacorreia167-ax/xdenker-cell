...
import { getProductsByCategories, COMPUTADORES_CATEGORIES } from "@/lib/queries";
import Link from "next/link";

const tabs: { label: string; value?: string }[] = [
    { label: "Todos" },
    { label: "MacBook", value: "macbook" },
    { label: "PC Gaming", value: "gaming" },
...
export default async function ComputadoresPage({
    ...
    const products = await getProductsByCategories(COMPUTADORES_CATEGORIES);
    ...
    return (
        <>
            ...
            <main className="px-4 lg:px-8 py-6 pb-28 lg:pb-12 max-w-7xl mx-auto">
                <h1 className="text-2xl font-bold mb-4">Computadores</h1>

                <div className="flex gap-2 mb-6 overflow-x-auto scrollbar-hide">
                    {tabs.map((tab) => (
                        <Link
                            key={tab.label}
                            href={tab.value ? `/computadores?cat=${tab.value}` : "/computadores"}
